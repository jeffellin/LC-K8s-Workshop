...
Alternatively store the values in a .env file - like [configurable.env](configurable.env). This is not the same as storing the config in YAML, because it's the native format and can be used outside of Kubernetes.
...
kubectl create configmap configurable-env-lab --from-env-file=~/exercises/labs/cm/lab/solution/configurable.env
...
- [override.json](override.json) has the required JSON settings. The filename is the same as the expected filename the app will read.

```
kubectl create configmap configurable-override-lab --from-file=~/exercises/labs/cm/lab/solution/override.json
...
```
kubectl apply -f ~/exercises/labs/cm/lab/solution/deployment-lab.yaml
```
